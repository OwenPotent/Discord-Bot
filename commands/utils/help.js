...
module.exports = {
    //definition
        ...
        category: "utils", //the category this will be listed at, for the help cmd
        aliases: ["h", "commandinfo"], //every parameter can be an alias or empty for no aliases
        cooldown: 1, //this will set it to a 5 second cooldown
...
const embed = new MessageEmbed() //defining the Embed
    ...
    .setTitle("HELP MENU")
    .setFooter(`To see command descriptions and information, type: ${config.prefix}help [Command Name]`, client.user.displayAvatarURL())
...
function getCMD(client,message,input){
    ...
    if(!cmd){ //if no cmd found return info no infos!
        return message.channel.send(embed.setColor("#87ceeb").setDescription(`No Information found for command **${input.toLowerCase()}**`));
    }
    ...
    //send the new Embed
    return message.channel.send(embed.setColor("#87ceeb"))
}
